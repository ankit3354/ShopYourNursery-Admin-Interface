...
const initialState: PlantsAdminState = {
  plants: [],
  loading: false,
  error: null,
};
...
const plantsAdminSlice = createSlice({
  ...
  reducers: {
    ...
    updatePlantsAdmin: (state, action) => {
      ...
      const findindex = state.plants.findIndex(
...
export const {
  setPlantsAdmin,
  addPlantsAdmin,
  removeFromPlantsAdmin,
  updatePlantsAdmin,
  setLoading,
  setError,
} = plantsAdminSlice.actions;
